...
from fixture_difficulty import get_short_team
import re
...
def get_home_stadium(team_code):
    ...
    if len(team) == 1:
        ...
        team = team.replace('3', 'Turf Moor, Burnley')
        team = team.replace('4', 'Vitality Stadium, Bournemouth')
        team = team.replace('5', 'Gtech Community Stadium, London')
        ...
        team = team.replace('17', 'Stadium Of Light, Sunderland')
...
def get_twitter_handle(team_code): 
    return get_short_team(team_code)
...
def get_team_badge(code):
    team_name = get_team_name(code)
    return team_badge_map.get(team_name, "")
